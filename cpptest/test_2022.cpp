...
1----.
ttdz -=0.05*dview;
./test_2022 plane.obj -c -s 0.8 -Y 160 -X 190 -a -5 -G -P -t -M /mmc/2022_3.mot
./playMotion2 /mmc/2022_3.mot 100

2----.
ttdz -=0.03*dview;
./test_2022 plane.obj -c -s 0.8 -Y 160 -X 190 -a -5 -G -P -t -M /mmc/2022_4.mot
./playMotion2 /mmc/2022_3.mot 100

./test_2022 plane.obj -c -s 0.8 -Y 160 -X 210 -z 2000 -a -5 -G -P -t -M /mmc/2022_E.mot


Note:
1. See marks of 'TEST:2202' as amendment to test_trimesh.cpp.
2. E3D_FLAT_SHADING --- Wall/Plane interference OK
   E3D_TEXTURE_MAPPING --- Wall/Plane interference WRONG!

Joural:
	2022-01-29: Create the file.
...
//	bool		flipXYZ_on =false;	/* Flip XYZ for screen coord */
	...
	/* For plane AABB Ty, for grid position */
	float Ty;

	/* TEST:2022 ---------- */
	E3D_RTMatrix  ttmat; /*  to transform localt TriGroup::omat.pmat[9-11] */
	float ttdz=0; /* For omat.pmat[11] as  DZ */
	bool meshWall_positioned=false;
	...
	/* P3. Prepare fbimg, and relates to FB.  fbimg is for motion_saving.  */
...
/* TEST:2022 ------- read wall2022.obj */
	E3D_TriMesh  meshWall("/mmc/3d/wall2022.obj");
	for(unsigned int k=0; k<meshWall.mtlList.size(); k++)
                        meshWall.mtlList[k].print();
...
/* TEST:2022 -------------------  */
	meshWall.scaleMesh(scale);
	meshWall.updateAllTriNormals();
	//meshWall.computeAllVtxNormals(); /* NOPE! for a box FLAT SAHDING. ! */
...
/* TEST:2022 ----------------- */
		meshWall.updateAABB();
		meshWall.moveAabbCenterToOrigin();
	...
	dview=3.0*meshModel.AABBdSize();  /* Taken dview == objSize */
...
/* TEST: -------- Get Ty */
	...
	/* 10. XXX Test multiline text texture */
	...
	meshWall.transformVertices(ttmat);
        ...
        for(unsigned int i=0; i< meshWall.triGroupList.size(); i++)   {
                 ...
                 */
                wtgOxyz.assign( meshWall.triGroupList[i].omat.pmat[9],
                                meshWall.triGroupList[i].omat.pmat[10],
                                 meshWall.triGroupList[i].omat.pmat[11] );

                wtgOxyz = wtgOxyz * ttmat;

                meshWall.triGroupList[i].omat.pmat[9] = wtgOxyz.x;
                meshWall.triGroupList[i].omat.pmat[10] = wtgOxyz.y;
                meshWall.triGroupList[i].omat.pmat[11] = wtgOxyz.z;

                /* pivot */
                meshWall.triGroupList[i].pivot = meshWall.triGroupList[i].pivot * ttmat;
        }
  #endif
...
//	workMesh->shadeType=E3D_GOURAUD_SHADING;
...
#if 1  /* TEST:2022 --------------- Transform TriGroup    */
	...
	ttmat.setTranslation(0.0, 0.0, ttdz);

	/* Transform veritces XYZ */
	workMesh->transformVertices(ttmat);

  #if 1       /* Transform TriGroup omat XXX NO USE!  workMesh be cloned each loop!!!  */
        E3D_Vector  tgOxyz;
        for(unsigned int i=0; i< workMesh->triGroupList.size(); i++)   {
                /* !!! ONLY transform TriGroup::omat.pmat[9-11](tx,ty,tz) as TriGroiup local origin.
                 *  tgRotMat is RELATIVE to its superior node,wh, NO NEED.
                 */
                tgOxyz.assign( workMesh->triGroupList[i].omat.pmat[9],
                                workMesh->triGroupList[i].omat.pmat[10],
                                 workMesh->triGroupList[i].omat.pmat[11] );

                tgOxyz = tgOxyz * ttmat;

                workMesh->triGroupList[i].omat.pmat[9] = tgOxyz.x;
                workMesh->triGroupList[i].omat.pmat[10] = tgOxyz.y;
                workMesh->triGroupList[i].omat.pmat[11] = tgOxyz.z;

                /* pivot */
                workMesh->triGroupList[i].pivot = workMesh->triGroupList[i].pivot * ttmat;
        }
  #endif
	/* Transform Normals */
//        workMesh->transformAllVtxNormals(ttmat);  /* normals should NOT apply scale_matrix ! */
//        workMesh->transformAllTriNormals(ttmat);  /* normals should NOT apply scale_matrix ! */
...
/* TEST:2022  ----------- transform ONLY once! as we set option: -a -5 */
	if(!meshWall_positioned) {
		/* meshWall, MORE z  */
		//VRTmat MUST be same as workMeh!
		meshWall.transformMesh(VRTmat, ScaleMat);
		meshWall_positioned=true;
	}
...
/* TEST:2022 ------------------------*/
        cout << "Render meshWall 2022 ... "<< endl;
	meshWall.shadeType=E3D_TEXTURE_MAPPING; // E3D_FLAT_SHADING;  E3D_GOURAUD_SHADING;
	meshWall.renderMesh(&gv_fb_dev, projMatrix);
	...
	if( showGrid_on ) {
		...
		/* Too big value of sx,sy to get out of the View_Frustum */
	        ...
	        E3D_draw_grid(&gv_fb_dev, 5.0*dSize, 5.0*dSize, dSize/10, gmat*VRTmat, projMatrix); /* fbdev sx,sy, us, VRTmat, projMatrix */
	}
	...
	sprintf(strtmp, "RotAngle: %.1f", angle); /* Rotation angle */
        ...
        FTsymbol_uft8strings_writeFB(   &gv_fb_dev, egi_appfonts.bold, /* FBdev, fontface */
                                        24, 24,                         /* fw,fh */
                                        (UFT8_PCHAR)"2022新春快乐!", 	/* pstr */
                                        // (UFT8_PCHAR)"E3D MESH", 	/* pstr */
                                        300, 1, 0,                      /* pixpl, lines, fgap */
                                        xres-175, yres-35,              /* x0,y0, */
                                        fontColor2, -1, 255,            /* fontcolor, transcolor,opaque */
                                        NULL, NULL, NULL, NULL );       /* int *cnt, int *lnleft, int* penx, int* peny */
