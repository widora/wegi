...
/*--------------------------------------------------------
Draw a dot with anti_aliasing effect, by calling function
draw_dot().

Note:
1. 4_Pixel Coverage: One sampling point will affect
...
{
	int ix=floor(x); /* Grid xy as for pixel coordinates. */
	int iy=floor(y);
...
/*-----------------------------------------------------
Draw a line by calling fdraw_dot().

TODO:
1. XXX For slope 45/135 deg, the line is much brighter!
   to adjust step accordingly!
2. Step=0.5 is 'the best' OR 'the ONLY right value'?
   concerning the '4_pixel coverage anti_aliasing algrithm'?
...
/*----------------------------------------------
draw a circle by calling fdraw_dot().
...
void fdraw_circle(FBDEV *dev, float x, float y, float r)
{
	...
	for(i=0;i<r;i+=0.5)  /* Step 0.5 is the BEST! */
	{
//		s=round(sqrt(1.0*r*r-1.0*i*i));
		s=sqrt(r*r-i*i);
//		if(i==0)s-=1; /* erase four tips */
		fdraw_dot(dev,x-s,y+i);
		fdraw_dot(dev,x+s,y+i);
		fdraw_dot(dev,x-s,y-i);
		fdraw_dot(dev,x+s,y-i);
		/* flip X-Y */
		fdraw_dot(dev,x-i,y+s);
		fdraw_dot(dev,x+i,y+s);
		fdraw_dot(dev,x-i,y-s);
		fdraw_dot(dev,x+i,y-s);
	}
...
void draw3D_line_simple(FBDEV *dev, int x1,int y1, int z1, int x2,int y2, int z2)
{
	...
	/* 1. An oblique line with X2>X1 */
	    ...
	    /* Traverse x1 (+)-> x2 */
            for(i=x1;i<=x2;i++) {
		/* j as pY */
                //j=(i-x1)*tekyy/tekxx+y1;		/* TODO: roundf() to improve accuracy. */
		j=roundf( 1.0*(i-x1)*tekyy/tekxx+y1 );
		if(y2>=y1) {
			/* Traverse tmp (+)-> pY */
			for(k=tmp;k<=j;k++) {		/* fill uncontinous points */
				//dev->pixz=z1+dist_points(x1,y1, i,k)*(z2-z1)/len;
				dev->pixz=roundf( z1+1.0*dist_points(x1,y1, i,k)*(z2-z1)/len );
		                draw_dot(dev,i,k);
			}
		}
		else { /* y2<y1, Traverse tmp (-)-> pY*/
	...
	/* 2. A vertical straight line X1==X2 */
	   ...
	   if(y2>=y1) {
		for(i=y1;i<=y2;i++) {
		   ...
		   dev->pixz=roundf( z1+1.0*dist_points(x1,y1, x1,i)*(z2-z1)/len );
		   draw_dot(dev,x1,i);
		}
	    }
	    else {
	...
	/* 3. An oblique line with X1>X2 */
		...
		/* j as pY */
                //j=(i-x2)*tekyy/tekxx+y2;
		j=roundf(1.0*(i-x2)*tekyy/tekxx+y2);
		if(y1>=y2) {
			/* Traverse tmp (+)-> pY */
			for(k=tmp;k<=j;k++)  {	/* fill uncontinous points */
			   	//dev->pixz=z2+dist_points(x2,y2, i,k)*(z1-z2)/len;
				dev->pixz=roundf( z2+1.0*dist_points(x2,y2, i,k)*(z1-z2)/len );
		        	draw_dot(dev,i,k);
			}
		}
		else {  /* y2>y1, Traverse tmp (-)-> pY */
...
//	EGI_16BIT_COLOR backcolor;
	...
	/* Save pixcolor */
        if(dev->pixcolor_on)    /* As private pixcolor */
                ...
                save_pixcolor=fb_color; /* We need this, see case PY IS an Integer! */
...
//	dev->pixcolor=save_pixcolor;  /* Assume save_pixcolor_on==true */
	if(save_pixcolor_on)   /* MidasHK_2022-04-26 */
		dev->pixcolor=save_pixcolor;
...
TODO:
1. float r = 0.5*w  and NEW fdraw_filled_triangle()
...
void draw_wline(FBDEV *dev,int x1,int y1,int x2,int y2, unsigned int w)
{
	...
	/* half width, also as circle rad */
	int r=w>>1; /* XXX so w=0 and w=1 is the same */
...
int draw_filled_rect2(FBDEV *dev, uint16_t color, int x1,int y1,int x2,int y2)
{
	...
	fbset_color2(dev, color); /* Hello U! MidasHK_2022-04-25 */
...
void draw_filled_triangle(FBDEV *dev, EGI_POINT *points)
{
	...
	/* If a vertical line. MidasHK_2022_03_11 */
	if(nl==nr) {
		//draw_pline_nc(dev, points, 3, 1);
		draw_line(dev, points[nl].x, points[nl].y, points[nr].x, points[nr].y);
		return;
	}
...
//	egi_dpstd("klr=%f, klm=%f, kmr=%f \n",klr,klm,kmr);
	...
	ymu=yu; /* So yu,yd sould NOT swap in above ! */
...
void draw_filled_triangle_outline(FBDEV *dev, EGI_POINT *points)
{
	...
	/* three points are collinear, vertical. */
	if(nl==nr) {
		draw_pline_nc(dev, points, 3, 1);
		return;
	}
...
@dev,		Pointer to FBDEV.
@x0-2,y0-2:	Coordinates of 3 points defining a triangle. FLOAT type.
...
//////////////////////////////
/*---------------------------------------------------
Same algrithm as draw_filled_triangle2/4().
To break down a filled triangles into pixels.

@x0/y0/z0-x2/y2/z2:  Triangle vertex points.
@pXYZ:		     To stored pixelated point X/Y/Z. autogrow
			       ...
			       float **pXYZ, size_t *capacity,  size_t *np)
{
	...
	/* For pixelation */
	float fpixz;
	int npp;  /* correspoind to *np */
	float *memXYZ;
	int growsize=256*3; /* growsize for pXYZ, 256 points. */


	/* Check input */
	if( pXYZ==NULL || *pXYZ==NULL || np==NULL || capacity==NULL )
		return -1;
	...
	*np=0;
	...
	if(x0==x1 && x1==x2 && y0==y1 && y1==y2) {
		...
		/* ----- pixelate the point ----- */
		fpixz = a*z0+b*z1+r*z2;

		/* Check capacity */
		if( (npp+1)*3 > *capacity) {
			memXYZ=realloc(*pXYZ, (*capacity+growsize)*sizeof(float));
			if(memXYZ==NULL)
				return 1;
		...
		/* Store XYZ */
		(*pXYZ)[3*npp]=x0; (*pXYZ)[3*npp+1]=y0; (*pXYZ)[3*npp+2]=fpixz;
		npp++;
		*np=npp;
...
//		egi_dpstd("fcheck~=0!, points: {%d,%d} {%d,%d} {%d,%d}\n", x0,y0,x1,y1,x2,y2);
	...
	if(nl==nr) {
		...
		for(k=yd; k<=yu; k++) {
			...
			/* Store XYZ */
			(*pXYZ)[3*npp]=x; (*pXYZ)[3*npp+1]=k; (*pXYZ)[3*npp+2]=fpixz;
			npp++;
			*np=npp;
				...
				for(k=tmp; k<=j; k++) {
					...
					//fb_dev->pixz = roundf(a*z0+b*z1+r*z2);
				     	//draw_dot(fb_dev,i,k);
		                        ...
		                        /* Store XYZ */
                		        (*pXYZ)[3*npp]=i; (*pXYZ)[3*npp+1]=k; (*pXYZ)[3*npp+2]=fpixz;
		                        npp++;
                		        *np=npp;
...
/*-----------------------------------------------------------------------
Draw a a filled triangle, pixel color are computed with barycentric
coordinates interpolation, with consideration of following cases:
...
/*------------------------------------------------
 ...
 TODO:
 1. XXX In some case, an end pixel is missing at left side
    of the center diameter
 2. To incorporate with draw_wline()
...
void draw_filled_circle(FBDEV *dev, int x, int y, int r)
{
	...
	for(i=0;i<r;i++)
	{
		/* Tricky MidasHK_2022_03_09 */
	   	if(r<2) {
			s=roundf(sqrt(r*r-i*i));
			s-=1; /* diameter is always odd */
		} else {
			s=roundf(sqrt(r*r-i*i)-0.5); /* r==1 will be a litte bigger */
	   	}

		//if(i==0)s-=1; /* erase four tips */
		if(i!=0) {	/* Otherwise it will call draw_line(dev,x-s,y,x+s,y) twice! */
			//draw_line(dev,x-s,y+i,x+s,y+i);
			draw_line_simple(dev,x-s,y+i,x+s,y+i); /* To avoid alpha effect */
		}
		draw_line_simple(dev,x-s,y-i,x+s,y-i);
	}

	/* Outer circle again for antialiasing effect.
	 * Note: draw_circle() and above draw_filled_circle algrithm is NOT consistent!
	 *   draw_circle() results a litter(MAYBE 1pixel more) bigger.
	 */
	if(r>1) {
	   if(dev->antialias_on) {
   #if 0 /* TEST: --------- */
   ...
   #endif
		fdraw_circle(dev,x,y,r);
	  }
	  else
		draw_circle(dev,x,y,r); /* To tender rim and make same as draw_circle */
	}
