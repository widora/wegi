...
#if 0	/* Client session process: Request and receive files */
	while(1) {
...
#else   /* Client session process: Request file list */
...
void* server_session_process(void *arg)
{
	...
	IHEAD_TYPE msgtype;
	LIST_LEAD  ListLead;
	FILE_LEAD *FileLead=NULL;	/* A pointer to ipack header */
...
while(1) {  ////////////////////////////     LOOP TEST     ///////////////////////////////
	...
	switch( msgtype ) {
		...
		case IHEAD_REQUEST_FILE:
			...
			FileLead=(FILE_LEAD *)IPACK_HEADER(ipack);
			//bzero(FileLead->fname, EGI_NAME_MAX);
...
printf("Create ipack...\n");
	/* Create ipack, make FILE_LEAD Header of the ipack. */
	ipack=inet_ipacket_create(sizeof(FILE_LEAD),NULL,0,NULL);     /* Only headsize, without privdata */
	if(ipack==NULL)
		return -2;

printf("Nest FileLead...\n");
	/* Nest FileLead into ipack, and put request file name. */
	FileLead=(FILE_LEAD *)IPACK_HEADER(ipack);
	snprintf(FileLead->fname, sizeof(FileLead->fname)-1, "%s", fname);
