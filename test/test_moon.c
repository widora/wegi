...
An example to display picture and moving text.
...
unsigned char utext[200]="WIDORA\n和小伙伴们:\n 中秋节快乐!";
int eboxW=120;
int offx=5;
int offy=5;

show_jpg("/tmp/moon.jpg",&gv_fb_dev, false, 0, 0);
do {    ///////////////////////     LOOP TEST 	////////////////////////

	/* For FTsymbols */
	memo_txt=egi_utxtdata_new( offx, offy,   		    /* offset from ebox left top */
                       		         2, 150,             	    /* lines, pixels per line */
		                         egi_appfonts.regular,      /* font face type */
                                	 24, 24,         	    /* font width and height, in pixels */
                                	 15,			    /* adjust gap */
...
memo_txt=egi_utxtdata_new( 13, offy,   		    /* offset from ebox left top */
              		   3, 150,             	    /* lines, pixels per line */
                           egi_appfonts.bold,       /* font face type */
                           24, 24,         	    /* font width and height, in pixels */
                           15,			    /* adjust gap */
			   WEGI_COLOR_FIREBRICK	    /* font color  */
                       	  );
...
ebox_txt= 	egi_txtbox_new(	"memo stick",   /* tag */
		                ...
		                WEGI_COLOR_YELLOW   /* prmcolor, -1 transparent*/
       			     );
ebox_txt->frame_alpha=120;

/* activate and display */
ebox_txt->activate(ebox_txt);
//egi_txtbox_unhide(ebox_txt);
...
while(1)
{
	...
	/* adjust box according to LCD pos_rotate */
        if(gv_fb_dev.pos_rotate % 2) {	/* pos_rotate=1,3 */
		pb=(EGI_POINT){319, 239};
		box.startxy=(EGI_POINT){0, 0};
		box.endxy=(EGI_POINT){pb.x-ebox_txt->width, pb.y-ebox_txt->height};
	} else {			/* pos_rotate=0,2 */
		pb=(EGI_POINT){239, 319};
		box.startxy=(EGI_POINT){0, 0};
		box.endxy=(EGI_POINT){pb.x-ebox_txt->width, pb.y-ebox_txt->height};
	}
