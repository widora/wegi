...
6. Should avoid more than one thread writing/rendering the surface!
 	...
 	egi_dpstd(DBG_GREEN"Page range offset [%d, %d]\n"DBG_RESET, chmap->pref-chmap->txtbuff,
		...
		switch( ch )
		{
				...
				if(enable_pinyin) {
					...
					if(chmap->maplines>2) {
						chmap->maplines -= (60/lndis); //2; HK2022-05-28
...
#if 0
        /* 1. Create System_V message queue */
        mqkey=ftok(".", 5);
