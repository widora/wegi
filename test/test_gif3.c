...
static bool DirectFB_ON=true;		/* -- FOR TEST ONLY --
					 ...
					 * to speed up writing!!
					 * 4. For big size image, it may result in tear lines and flash(by FILO)
					 *    on the screen.
					 * 5. For images with transparent area, it MAY flash with bkgcolor!(by FILO)
					 *
					 */
...
static int  trans_color=-1;     	/* Palette index for transparency, -1 if none, or NO_TRANSPARENT_COLOR
					  * Defined in gif file.
					 */
static int  user_trans_color=-1;        /* -- FOR TEST ONLY --
					 * Usually disable it with -1.
...
while(1) {	/////////////////////    OUTER LOOP TEST    ///////////////////////

    egif=egi_gif_readfile(FileName);
    if(egif==NULL)
	   exit(-1);

    SWidth=egif->SWidth;
    SHeight=egif->SHeight;
...
//while(1) {	/////////////////////    LOOP TEST    ///////////////////////
    ...
    for( i=0; i< egif->ImageCount; i++ )
    {
	printf("\nGet ImageData[%d] ...\n", i);
        //ImageData=&GifFile->SavedImages[i];
        ImageData=&egif->SavedImages[i];
	...
	/* <<< Display image >> */
        /* WARING!!! Switch DirectFB only after gif finish drawing it canvan, usually is the first image[0] */
